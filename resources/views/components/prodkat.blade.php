...
@foreach ($kategori as $kat)
    <div class="tab-pane fade" id="nav-{{ $kat->slug }}" role="tabpanel" aria-labelledby="nav-{{ $kat->slug }}-tab"
        tabindex="0">
        <div class="tpproduct__arrow p-relative">
            <div class="swiper-container tpproduct-active-custom"
                data-loop="{{ $kat->Product->count() >= 3 ? 'true' : 'false' }}">
                <div class="swiper-wrapper">
                    @forelse ($kat->product as $produk)
                        <div class="swiper-slide">
                            <div class="tpproduct p-relative">
                                <div class="tpproduct__thumb p-relative text-center">
                                    <a href="#"><img src="{{ $produk->getMedia('foto_product')->first()?->getUrl() }}"
                                            alt=""></a>
                                    <a class="tpproduct__thumb-img" href="shop-details.html"><img
                                            src="{{ $produk->getMedia('foto_product')->get(1)?->getUrl() }}" alt=""></a>
                                    <div class="tpproduct__info bage">
                                        <span class="tpproduct__info-discount bage__discount">{{$produk->diskon}}%</span>
                                        <span
                                            class="tpproduct__info-hot bage__hot">{{ $produk->label == 1 ? 'SUPER MURAH🔥' : '' }}</span>
                                    </div>
                                ...
                                <div class="tpproduct__hover-text">
                                    <div class="tpproduct__hover-btn d-flex justify-content-center mb-10">
                                        <livewire:add-to-cart :productId="$produk->id" />
                                    </div>
